...
public class ChatNoirModel {
	...
	private Block referencePoint;

	private ArrayList<ArrayList<Boolean>> bools;

	private ArrayList<Block> blocksInQueue;
	...
	public int[] getcatCoordinates() {
		return getBlockCoordinates(catPosition);
	}
...
//		if (gameOver() == true) {
//			throw new Exception("La da da da da");
//		}
...
//	}
	...
	private boolean hasPath() {
		...
		if (firstTime == true) {
			referencePoint = blocks.get(getBlockCoordinates(catPosition)[0]).get(getBlockCoordinates(catPosition)[1]);
			queue.add(referencePoint);
			bools = new ArrayList<>();
			blocksInQueue = new ArrayList<Block>();
			for (int i = 0; i < blocks.size(); i++) {
		...
		while (queue.isEmpty() == false) {
			...
			for (int i = 0; i < 6; i++) {
				if (adjacentBlocks.get(i).isEdge())
					return true;
				else if (!adjacentBlocks.get(i).containsWall() && !adjacentBlocks.get(i).isEdge()) {
					...
					if (bools.get(adjacentBlockCoordinates[0]).get(adjacentBlockCoordinates[1]) == false) {
						if (!blocksInQueue.contains(adjacentBlocks.get(i))) {
							queue.add(adjacentBlocks.get(i));
							blocksInQueue.add(adjacentBlocks.get(i));
						}
					}
